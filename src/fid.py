...
class FiD(T5ForConditionalGeneration):
    ...
    def get_crossattention_scores(self, n_passages, mask, labels, ids, mode="all", mask_query=None):
        ...
        if "norms" in mode or "all" in mode or "ppmean" in mode:
            self.aggregate_value(norms, mask, labels, n_passages, ids, mask_query, output, prefix="norms")
        return output

    def get_crossattention_scores_lm(self, n_passages, mask, labels, ids, mode="all", mask_query=None):
        """
        Cross-attention scores are aggregated to obtain a single scalar per
        passage. This scalar can be seen as a similarity score between the
        question and the input passage. It is obtained by averaging the
        ...
        scores, norms, probs = [], [], []
        for mod in self.decoder.block:
            scores.append(mod.layer[1].EncDecAttention.score_storage_lm)
            norms.append(mod.layer[1].EncDecAttention.normalized_score_storage_lm)
            probs.append(mod.layer[1].EncDecAttention.prob_storage_lm)
        scores = torch.stack(scores)
        norms = torch.stack(norms)
        probs = torch.stack(probs)
    ...
    def create_crossattention_storage_lm(self):
        for mod in self.decoder.block:
            xattn = mod.layer[1].EncDecAttention
            xattn.score_storage_lm = None
            xattn.normalized_score_storage_lm = None
            xattn.prob_storage_lm = None
...
def cross_attention_forward(
    ...
    if hasattr(self, "score_storage_lm"):
        self.score_storage_lm = scores.mean(dim=1)
        self.prob_storage_lm = attn_weights.mean(dim=1)
        self.normalized_score_storage_lm = (
            (torch.norm(value_states.float(), dim=-1)[:, :, None] * attn_weights).mean(dim=1)
        )
