...
def split_fc_text(pas):
    fc_passage = pas.replace("Misinformation isn\'t going away just because it\'s a new year. Support\n   trusted, factual information with a tax deductible contribution to\n   PolitiFact.","")
    text = fc_passage.split(" ")
    split_text = [" ".join(text[i : i + 100]).strip() for i in range(0, len(text), 100)]
    return split_text

def split_fc_pas(id, pas):
    split_passages = []
    split_text = split_fc_text(pas)
    for pi,passage in enumerate(split_text):
        split_passages.append({'id':-int(id)*10-pi,'title':"","text":passage})
